...
HRESULT CEdit::Ready_Layer_Player(const _wstring& strLayerTag)
{
...
HRESULT CEdit::Ready_Layer_Monster(const _wstring& strLayerTag)
{
        std::ifstream ifs("../../Mapdata/scene.txt");
        if (!ifs.is_open())
        {
                if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::EDIT), strLayerTag,
                        ENUM_CLASS(LEVEL::EDIT), TEXT("Prototype_GameObject_Monster"))))
                        return E_FAIL;
                return S_OK;
        }
		struct MapObject
			...
			>> obj.pos[0] >> obj.pos[1] >> obj.pos[2])
		{
			...
			if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::EDIT), strLayerTag,
				ENUM_CLASS(LEVEL::EDIT), TEXT("Prototype_GameObject_Monster"), &desc)))
