...
CBoss_Mask::CBoss_Mask(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CGameObject{ pDevice, pContext } {
}

CBoss_Mask::CBoss_Mask(const CBoss_Mask& Prototype)
    : CGameObject{ Prototype } {
}

HRESULT CBoss_Mask::Initialize_Prototype() { return S_OK; }

HRESULT CBoss_Mask::Initialize(void* pArg)
{
    ...
    if (pArg)
    {
        auto* pDesc = static_cast<Boss_Mask*>(pArg);
        m_eType = pDesc->type;
...
void CBoss_Mask::Priority_Update(_float) {}

void CBoss_Mask::Update(_float dt)
{
}

void CBoss_Mask::Late_Update(_float)
{
    m_pGameInstance->Add_RenderGroup(RENDERGROUP::NONBLEND, this);
}

HRESULT CBoss_Mask::Render()
{
...
HRESULT CBoss_Mask::Bind_ShaderResources()
{
...
CBoss_Mask* CBoss_Mask::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    auto* p = new CBoss_Mask(pDevice, pContext);
    if (FAILED(p->Initialize_Prototype())) { MSG_BOX(TEXT("Failed to Created : CBoss_Mask")); Safe_Release(p); }
    return p;
}

CGameObject* CBoss_Mask::Clone(void* pArg)
{
    auto* p = new CBoss_Mask(*this);
    if (FAILED(p->Initialize(pArg))) { MSG_BOX(TEXT("Failed to Clone : CBoss_Mask")); Safe_Release(p); }
    return p;
}
