...
CHero_Intro::CHero_Intro(const CHero_Intro& Prototype)
    : CContainerObject{ Prototype } {
}

HRESULT CHero_Intro::Initialize_Prototype() { return S_OK; }

HRESULT CHero_Intro::Initialize(void* pArg)
{
    ...
    CPlayerManager::GetInstance()->Register(ENUM_CLASS(LEVEL::INTRO), 0, this, 100.f);
    CPlayerManager::GetInstance()->SetActive(ENUM_CLASS(LEVEL::INTRO), 0);

    return S_OK;
}

void CHero_Intro::Priority_Update(_float fTimeDelta)
{
...
void CHero_Intro::Late_Update(_float fTimeDelta)
{
    // 네비 위 보정
...
HRESULT CHero_Intro::Render()
{
    return S_OK;
}

// --- Helpers ---
_vector CHero_Intro::Get_TransformState(Engine::STATE s) const
{
    return m_pTransformCom ? m_pTransformCom->Get_State(s) : XMVectorZero();
}

_vector CHero_Intro::GetPos()    const { return Get_TransformState(Engine::STATE::POSITION); }
_vector CHero_Intro::GetRight()  const { return XMVector3Normalize(Get_TransformState(Engine::STATE::RIGHT)); }
_vector CHero_Intro::GetUp()     const { return XMVector3Normalize(Get_TransformState(Engine::STATE::UP)); }
_vector CHero_Intro::GetForward(bool flattenY) const
{
    _vector f = Get_TransformState(Engine::STATE::LOOK);
    return flattenY ? XMVector3Normalize(XMVectorSetY(f, 0.f)) : XMVector3Normalize(f);
}

void CHero_Intro::Throw_Spear()
{
    ...
    CObject_Pool_Manager::GetInstance()
        ->Acquire(LEVEL::INTRO, L"Layer_Spear", &desc);
}

HRESULT CHero_Intro::Ready_Components()
{
    CNavigation::NAVIGATION_DESC NaviDesc{};
    NaviDesc.iCurrentCellIndex = 0;

    if (FAILED(CGameObject::Add_Component(
        ENUM_CLASS(LEVEL::INTRO), TEXT("Prototype_Component_Navigation_Intro"),
...
HRESULT CHero_Intro::Ready_PartObjects()
{
    CBody_Into::BODY_DESC BodyDesc{};
    ...
    if (FAILED(__super::Add_PartObject(TEXT("Part_Body"),
        ENUM_CLASS(LEVEL::INTRO), TEXT("Prototype_GameObject_Body_Intro"), &BodyDesc)))
        return E_FAIL;

    CPartObject* pBody = Find_PartObject(TEXT("Part_Body"));
    if (!pBody) return E_FAIL;

    CWeapon_Intro::WEAPON_DESC WDesc{};
    WDesc.pState = &m_iState;
    WDesc.pSocketMatrix = dynamic_cast<CBody_Into*>(pBody)->Get_BoneMatrix("jnt_Spine_01_SKN");
    WDesc.pSocketMatrix_Hand = dynamic_cast<CBody_Into*>(pBody)->Get_BoneMatrix("jnt_weapon");
    ...
    if (FAILED(__super::Add_PartObject(TEXT("Part_Weapon"),
        ENUM_CLASS(LEVEL::INTRO), TEXT("Prototype_GameObject_Weapon_Intro"), &WDesc)))
        return E_FAIL;
...
// ====== 충돌/트리거 유틸 구현 ======
bool CHero_Intro::CheckBlockingWithLayer(const _wstring& layerName) const
{
    const _uint level = ENUM_CLASS(LEVEL::INTRO);
...
bool CHero_Intro::CheckTriggerWithLayer(const _wstring& layerName) const
{
    const _uint level = ENUM_CLASS(LEVEL::INTRO);
...
bool CHero_Intro::ResolveBlockingCollisions()
{
...
void CHero_Intro::TickDamageTriggers(float dt)
{
    m_damageTickAcc += dt;
...
void CHero_Intro::ApplyDamage(int amount)
{
...
void CHero_Intro::TickHostileHits(float dt)
{
    auto* pm = CPlayerManager::GetInstance();
...
{
    CHero_Intro* pInstance = new CHero_Intro(pDevice, pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed to Created : CHero_Intro"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

CGameObject* CHero_Intro::Clone(void* pArg)
{
    CHero_Intro* pInstance = new CHero_Intro(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Failed to Clone : CHero_Intro"));
        Safe_Release(pInstance);
    }
    return pInstance;
}
