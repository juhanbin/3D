...
CBoss_Hand_L::CBoss_Hand_L(ID3D11Device* d, ID3D11DeviceContext* c) : CGameObject(d, c) {}
CBoss_Hand_L::CBoss_Hand_L(const CBoss_Hand_L& r) : CGameObject(r) {}
...
HRESULT CBoss_Hand_L::Ready_Components()
{
    ...
    if (FAILED(Add_Component(ENUM_CLASS(LEVEL::BOSS), TEXT("Prototype_Component_Model_Boss_hand_L"),
...
void CBoss_Hand_L::StartSlam(const _float3& targetWorld)
{
...
void CBoss_Hand_L::TickSlam(_float dt)
{
    if (m_slam == SLAM::NONE) return;
...
// ===== 생성/클론 =====
CBoss_Hand_L* CBoss_Hand_L::Create(ID3D11Device* d, ID3D11DeviceContext* c)
{
    auto* p = new CBoss_Hand_L(d, c);
    if (FAILED(p->Initialize_Prototype())) { MSG_BOX(L"Failed to Created : CBoss_Hand_L"); Safe_Release(p); }
    return p;
}
CGameObject* CBoss_Hand_L::Clone(void* pArg)
{
    auto* p = new CBoss_Hand_L(*this);
    if (FAILED(p->Initialize(pArg))) { MSG_BOX(L"Failed to Clone : CBoss_Hand_L"); Safe_Release(p); }
    return p;
}
