...
CLevle_Bridge::CLevle_Bridge(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLevel{ pDevice, pContext }
{
}

HRESULT CLevle_Bridge::Initialize()
{
	...
	m_SceneObjects = LoadSceneObjects("../../Mapdata/scene_bridge.bin");
...
void CLevle_Bridge::Update(_float fTimeDelta)
{
...
HRESULT CLevle_Bridge::Render()
{
	SetWindowText(g_hWnd, TEXT("다리레벨입니다."));

	return S_OK;
}

vector<MapObject> CLevle_Bridge::LoadSceneObjects(const char* file)
{
...
void CLevle_Bridge::Pool_Initialize() {
	auto* pool = CObject_Pool_Manager::GetInstance();

	HRESULT hr = pool->Register_Pool(
		LEVEL::BRIDGE, L"Layer_Spear", 64,
		std::bind(&CLevle_Bridge::CreateSpear_ForPool, this));
#ifdef _DEBUG
	if (FAILED(hr)) OutputDebugStringW(L"[POOL] Spear Register_Pool 실패\n");
#endif

	hr = pool->Register_Pool(
		LEVEL::BRIDGE, L"Layer_Arrow", 64,
		std::bind(&CLevle_Bridge::CreateArrow_ForPool, this));
#ifdef _DEBUG
...
Engine::CGameObject* CLevle_Bridge::CreateSpear_ForPool()
{
	...
	CGameInstance::GetInstance()
		->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), L"Layer_Spear", p);

	p->Set_Active(false);
	return p;
}

Engine::CGameObject* CLevle_Bridge::CreateArrow_ForPool()
{
	CGameObject* p = static_cast<CGameObject*>(
		CGameInstance::GetInstance()->Clone_Prototype(
			PROTOTYPE::GAMEOBJECT,
	...
	CGameInstance::GetInstance()->Add_GameObject_ToLayer(
		ENUM_CLASS(LEVEL::BRIDGE), L"Layer_Arrow", p);

	p->Set_Active(false);
	return p;
}

HRESULT CLevle_Bridge::Ready_Lights()
{
...
HRESULT CLevle_Bridge::Ready_Layer_Camera(const _wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Camera_Free"), &CameraDesc)))
		return E_FAIL;

	return S_OK;
}

HRESULT CLevle_Bridge::Ready_Layer_Camera_Player(const _wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(
		ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Camera_Player"), &CameraDesc)))
		return E_FAIL;

	return S_OK;
}

HRESULT CLevle_Bridge::Ready_Layer_BackGround(const _wstring& strLayerTag)
{
	/*if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Terrain_BRIDGE"))))
		return E_FAIL;*/

	/*if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Sky"))))
		return E_FAIL;*/

		/*if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
			ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Bridge"))))
			return E_FAIL;*/

	/*m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_Component_Texture_Sky_Bridge"));*/
...
HRESULT CLevle_Bridge::Ready_Layer_Player(const _wstring& strLayerTag)
{
	for (auto& obj : m_SceneObjects)
	{
		if ((EObjectType)obj.type == EObjectType::HERO)
		{
			...
			if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(
				ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
				ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Player_Bridge"), &desc)))
			{
...
HRESULT CLevle_Bridge::Ready_Layer_MapObjects(const _wstring& strLayerTag)
{
	for (auto& obj : m_SceneObjects)
	{
		if ((EObjectType)obj.type == EObjectType::CELLING_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
			desc.vRot = _float3(obj.rot[0], obj.rot[1], obj.rot[2]);
			desc.vPos = _float3(obj.pos[0], obj.pos[1], obj.pos[2]);
			...
			if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(
				ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
				ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_MapObject_Bridge"), &desc)))
			{
		...
		if ((EObjectType)obj.type == EObjectType::MOD_VAULTED_CORRIDOR_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_VAULTED_STAIR_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_VAULTED_BROKEN_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_VAULTED_BROKEN_AB)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_BRIDGEGROUND_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_BRIDGEARCH_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::MOD_BRIDGEBROKEN_AA)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::SM_MERGED_MOD_THREEDEFENSES)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
		...
		if ((EObjectType)obj.type == EObjectType::DOOR)
		{
			CMapObject_Bridge::MAPOBJECT_DESC desc{};
			desc.type = static_cast<EObjectType>(obj.type);
			desc.vScale = _float3(obj.size[0], obj.size[1], obj.size[2]);
...
HRESULT CLevle_Bridge::Ready_Layer_Monster(const _wstring& strLayerTag)
{
	for (auto& obj : m_SceneObjects)
		{
			if ((EObjectType)obj.type == EObjectType::MONSTER || (EObjectType)obj.type == EObjectType::SKELETON_SPEAR || (EObjectType)obj.type == EObjectType::SKELETON_BOW )
			{
				...
				if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(
					ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
					ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Skeleton_Bridge"), &desc)))
				{
...
HRESULT CLevle_Bridge::Ready_Layer_Effect(const _wstring& strLayerTag)
{
	/*if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Particle"))))
		return E_FAIL;

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
		ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Snow"))))
		return E_FAIL;

	for (size_t i = 0; i < 50; i++)
	{
		if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
			ENUM_CLASS(LEVEL::BRIDGE), TEXT("Prototype_GameObject_Effect_Explosion"))))
			return E_FAIL;
...
HRESULT CLevle_Bridge::Ready_Layer_UI(const _wstring& strLayerTag)
{
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::BRIDGE), strLayerTag,
...
CLevle_Bridge* CLevle_Bridge::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLevle_Bridge* pInstance = new CLevle_Bridge(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(TEXT("Failed to Created : CLevle_Bridge"));
